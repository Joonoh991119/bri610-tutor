...
_REVIEW_KIMI       = "moonshotai/kimi-k2.6"          # paid, cost-effective deep reasoner
...
ROUTES: dict[str, RouteSpec] = {
    # Routing principle (per user mandate 2026-04-27): default chat/summary/derive
    # to **cost-effective deep-reasoning** OpenRouter models — DeepSeek v4 pro
    # and Kimi K 2.6 — instead of qwen3.6-plus. Free tier qwen kept ONLY for
    # cheap utility calls (router, persona, diagnostic).
    "router":           RouteSpec("router",           _OR_QWEN35_PLUS,  _OR_QWEN36_PLUS, _OLL_QUICK,   0.0,  10),
    "tutor":            RouteSpec("tutor",            _REVIEW_DEEPSEEK, _REVIEW_KIMI,    _OLL_PRIMARY, 0.65, 4096),  # DeepSeek v4 pro PRIMARY
    "derive":           RouteSpec("derive",           _REVIEW_DSR1,     _REVIEW_DEEPSEEK,_OLL_MATH,    0.3,  4096),  # DeepSeek-R1 chain-of-thought
    "consultant":       RouteSpec("consultant",       _REVIEW_DEEPSEEK, _PAID_SONNET,    _OLL_PRIMARY, 0.0,  300),
    "explain_my_answer":RouteSpec("explain_my_answer",_REVIEW_KIMI,     _REVIEW_DEEPSEEK,_OLL_PRIMARY, 0.5,  1500),
    "quiz_generator":   RouteSpec("quiz_generator",   _REVIEW_KIMI,     _REVIEW_DEEPSEEK,_OLL_PRIMARY, 0.5,  3000),
    "priority_scorer":  RouteSpec("priority_scorer",  _REVIEW_KIMI,     _OR_QWEN36_PLUS, _OLL_PRIMARY, 0.0,  500),
    # Lens reviewers — cross-model auditing
    "lens_factual":     RouteSpec("lens_factual",     _REVIEW_DEEPSEEK, _PAID_SONNET,    _OLL_PRIMARY, 0.0,  700),     # DeepSeek v4 pro factual veto
    "lens_pedagogical": RouteSpec("lens_pedagogical", _REVIEW_KIMI,     _REVIEW_DEEPSEEK,_OLL_PRIMARY, 0.0,  500),     # Kimi K2.6 pedagogy
    "lens_korean":      RouteSpec("lens_korean",      _OR_QWEN36_PLUS,  _REVIEW_KIMI,    _OLL_PRIMARY, 0.0,  400),     # Korean strongest on qwen
    "lens_difficulty":  RouteSpec("lens_difficulty",  _REVIEW_KIMI,     _REVIEW_DEEPSEEK,_OLL_PRIMARY, 0.0,  300),
    "persona_narrator": RouteSpec("persona_narrator", _REVIEW_KIMI,     _OR_QWEN36_PLUS, _OLL_PRIMARY, 0.55, 700),     # Kimi 한국어 자연스러움
    "summary":          RouteSpec("summary",          _REVIEW_DEEPSEEK, _REVIEW_KIMI,    _OLL_PRIMARY, 0.45, 4500),    # DeepSeek v4 pro for elevated concept maps
    "diagnostic":       RouteSpec("diagnostic",       _OR_QWEN35_PLUS,  _REVIEW_KIMI,    _OLL_QUICK,   0.0,  300),
