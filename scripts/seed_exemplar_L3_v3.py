#!/usr/bin/env python3
"""
seed_exemplar_L3_v3.py — v3 of L3 summary.

v3 changes (per feedback_v3_summaries.md):
  - Stripped specific numerical anchors (C_m value, R_m value, tau_m range,
    E_K/E_Na numeric anchors, concentration values, calibration anecdotes)
  - Added 3 <details> toggles before demanding derivation steps
...
이 사실 하나로 "왜 막에 전압이 존재하는가" 가 설명된다. Capacitor 에 전하 $Q$ 가 분리되면 그 사이에 전압 $V = Q/C$ 가 자동으로 생긴다. 신경막의 두께가 *매우 작기* 때문에 단위 면적당 정전용량 $C_m$ 이 극단적으로 크고, 따라서 *작은* 전하 불균형으로도 *큰* 막전위를 만들 수 있다.

Bilayer 두께가 진화적으로 보존되었기 때문에 $C_m$ 은 세포 종류에 거의 무관하게 일정하다 [Slide L3 p.18].
...
<details>
<summary><em>(펼쳐서 복습) $Q = CV$ 에서 $I_C = C\,dV/dt$ 가 처음이라면</em></summary>

$Q = CV$ 의 양변을 시간 $t$ 에 대해 미분하면 $dQ/dt = C\,dV/dt$. 전류의 정의 $I = dQ/dt$ 를 대입하면 $I_C = C\,dV/dt$. 자세한 유도: Foundation 카드 §capacitor 전류 정의.
</details>
...
$R_m$ 은 $C_m$ 과 달리 *세포마다 극단적으로 다르다*. 같은 종류의 뉴런이라도 K leak channel 발현량에 따라 10×–1000× 차이가 날 수 있다. 이 변동성이 "같은 입력에 다르게 반응하는 세포 종류" 의 본질이다 [Slide L3 p.22]. specific membrane resistance 는 세포 종류에 따라 크게 달라지므로, 단일 값으로 외우기보다 *왜 다른가* 를 이해하는 것이 중요하다.
...
휴지에서 $g_K \gg g_{Na}$ (K leak 채널 우세) 이므로 $V_\infty$ 는 $E_K$ 에서 약간만 양의 방향으로 끌려 $V_\mathrm{rest}$ 에 안착한다. 이 *작은 차이* 가 Na 누설의 직접적 결과이다 [Slide L3 p.30].
...
<details>
<summary><em>(펼쳐서 복습) KCL 적용이 처음이라면</em></summary>

KCL: 한 노드에서 나가는 전류의 합 = 들어오는 전류의 합. 그림 3 의 노드 $V$ 에서: 들어오는 $I_{inj}$ = capacitor 로 나가는 $I_C$ + 저항으로 나가는 $I_R$. Foundation 카드 §KCL 참조.
</details>
...
*Step input 의 폐형 해*: $V(t) = V_\infty + (V_0 - V_\infty)\,e^{-t/\tau_m}$, 단 $V_\infty = V_{rest} + R_m I_{inj}$.

<details>
<summary><em>(펼쳐서 복습) 분리변수법이 처음이라면</em></summary>

$\tau_m\,du/dt = -u$ (여기서 $u = V - V_\infty$) 는 변수분리형 ODE: $du/u = -dt/\tau_m$. 양변 적분 → $\ln|u| = -t/\tau_m + C$ → $u(t) = u(0)\,e^{-t/\tau_m}$. Foundation 카드 §1차 ODE (동질) 참조.
</details>
...
휴지 상태 막은 K leak channel 이 dominant 이므로 $V_\mathrm{rest} \approx E_K$ — 이것이 "왜 휴지 막이 음수인가" 의 답이다. $K^+$ 의 세포 내외 농도 비율이 크기 때문에 $E_K$ 는 강한 음의 값을 가진다.
...
| **$R_m$** | sub-threshold step current 의 *정상상태* 전압 측정 → $\Delta V / I$ | 매우 짧은 transient — 정상상태 도달 안 함, capacitor 가 dominant |
...
if __name__ == "__main__":
    ...
    chars = len(L3_SUMMARY)
    figs = sum(1 for t in ["__F1__", "__F2__", "__F3__", "__F4__", "__F5__"] if t not in L3_SUMMARY)
    print(f"L3 v3 cached: {chars} chars; figures: 5; toggles: 3; sections: 11")
