#!/usr/bin/env python3
"""
seed_exemplar_L4_v3.py — v3 of L4 summary.

v3 changes (per feedback_v3_summaries.md):
  - Stripped specific numerical anchors (V_rest numeric value, release prob range,
    NT diffusion time, PSTH window values, vesicle fusion probability)
  - Added 3 <details> toggles before KCL re-derivation, alpha-function reasoning,
    and conductance-based current
...
F5 = fig("synapse_chemical.svg",
         ...
         "그림 5. AP → Ca²⁺ 채널 개방 → vesicle exocytosis → NT 확산 → 수용체 결합 → 이온 흐름의 6 단계 [Slide L4 p.15–17].")
...
<details>
<summary><em>(펼쳐서 복습) KCL 과 $I_C = C\,dV/dt$ 가 처음이라면</em></summary>

KCL: 한 노드로 들어오는 전류 합 = 나가는 전류 합. Capacitor 전류는 $I_C = C\,dV/dt$ ($Q = CV$ 미분), 저항 전류는 Ohm 의 법칙 $I_R = (V - V_\text{rest})/R_m$. Foundation 카드 §KCL + §capacitor 전류 참조.
</details>
...
GHK 식 (L3 §8) 에서 $p_K \gg p_\text{Na}$ 이므로 $V_\text{rest} \approx E_K$. Cardiac pacemaker 처럼 leakage Na 가 과발현된 세포는 $V_\text{rest}$ 가 올라가 자발 발화에 가까워진다.

**Pump 의 역할**: leakage 만 있으면 농도 기울기 무너져 $V_\text{rest}\to 0$. Na/K-ATPase 가 1 ATP 당 3 Na 밖 / 2 K 안 으로 기울기 유지. **ATP 차단 → $V_\text{rest}$ 붕괴 (ischemic depolarization)** — L3 §6 "능동 평형" 의 분자 정체.
...
**한 줄 요약.** AP 도착 → Ca²⁺ 채널 개방 → vesicle exocytosis → NT 확산 → 수용체 결합 → 이온 흐름. 각 단계에 수십~수백 μs 의 지연이 있다 [Slide L4 p.15–17].

**가장 큰 지연 두 곳**:
- **Ca²⁺ 의존 vesicle 융합** — 확률적. AP 하나가 도착해도 vesicle 이 반드시 분비되지는 않는다.
- **NT 시냅스 틈 확산** — 좁은 틈을 Brownian 확산으로 건너는 시간.
...
| **NMDA** | Na+K+Ca | ≈ 0 mV | Mg²⁺ 가 휴지 전위 근방에서 차단; *탈분극 후*에만 통과 |
...
**한 줄 요약.** 단일 시냅스 conductance: $g_\text{syn}(t) = A\,t\,e^{-t/t_\text{peak}}$. 피크 후 지수 감소로 수 ms 에 걸친 전도 변화를 기술한다 [Slide L4 p.27].

<details>
<summary><em>(펼쳐서 복습) alpha-function 의 극값 조건이 처음이라면</em></summary>

$g(t) = At\,e^{-t/t_\text{peak}}$ 의 극대 위치: $dg/dt = A(1 - t/t_\text{peak})e^{-t/t_\text{peak}} = 0$ → $t = t_\text{peak}$. 즉 함수 이름이 된 매개변수가 그대로 피크 시각이다. Foundation 카드 §지수함수 미분 참조.
</details>
...
<details>
<summary><em>(펼쳐서 복습) conductance-based 시냅스 전류가 처음이라면</em></summary>

시냅스 전류 = $I_\text{syn} = g_\text{syn}(t)(V - E_\text{syn})$. $g_\text{syn}$ 이 시간에 따라 변하므로 이 항은 *선형이 아닌* (V 와 g 가 곱) 비선형 기여다. $V = E_\text{syn}$ 에서 전류가 0이 되는 것이 reversal potential 의 정의. Foundation 카드 §driving force 참조.
</details>
...
2. **"GABA = 항상 inhibitory"** — 아니다. 발달기 미성숙 뉴런은 $E_\text{Cl}$ 이 덜 음수 → GABA_A 가 *탈분극* (KCC2 발현 전).
3. **"시냅스 = 결정론적"** — AP 당 release 확률이 1 이 아니다. 결정적으로 보이는 것은 통계 평균.
4. **"$g_\text{syn}$ 곡선 = PSP 곡선"** — PSP 는 $g_\text{syn}$ 을 $\tau_m$ lowpass 필터한 결과.
5. **"Pump 가 막전위를 직접 만든다"** — Pump 는 농도 기울기 생성, leakage 가 막전위 생성. Pump electrogenicity 는 소량만 기여.
...
if __name__ == "__main__":
    ...
    chars = len(L4_SUMMARY)
    print(f"L4 v3 cached: {chars} chars; figures: 6; toggles: 3; sections: 13")
