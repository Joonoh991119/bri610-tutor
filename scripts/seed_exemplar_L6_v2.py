#!/usr/bin/env python3
"""
seed_exemplar_L6_v2.py — Opus 4.7 hand-rewritten L6 summary, v2.

Cable theory + AP propagation. Plain Korean + inline figures.
"""
import json, sys
...
F2 = fig("cable_decay_spatial.svg",
         ...
         "그림 2. Semi-infinite cable 에 x=0 에서 일정 전류 주입 시 정상상태 막전위. V(x) = V₀ · exp(-x/λ) — 거리 λ 마다 1/e ≈ 37% 로 감쇠. λ ≈ 0.1–1 mm 가 전형값. 이 *공간 감쇠* 가 dendritic input 의 attenuation 본질.")

F3 = fig("ap_propagation_unmyelinated.svg",
         "Unmyelinated axon — AP propagation as a wave",
         "그림 3. Unmyelinated axon 의 AP 전파. 한 지점의 depolarization 이 *cable current* 로 옆 지점을 임계 위로 끌어올림 → Na 채널 재점화 → 새 AP. 결과: AP 가 *감쇠 없이* 일정 진폭으로 전파 (passive 감쇠 ≠ AP). 속도 ~0.4 m/s.")

F4 = fig("ap_propagation_myelinated.svg",
         "Myelinated axon — saltatory conduction",
         "그림 4. Myelin 으로 절연된 internode 는 capacitor 가 *얇아지고* leak 이 줄어든 cable. AP 가 Node of Ranvier 에서만 재점화 — *건너뛰는* (saltatory) 전파. 속도 50–100 m/s, 100–250 배 빠름.")
...
> **24-시간 마스터리 목표.** (i) cable PDE 를 KCL + Ohm + capacitor 로 30초 안에 유도하고, (ii) 정상상태 해 V(x) = V₀·e^{-x/λ} 와 λ = √(d R_m / 4 R_i) 의 *물리적 의미* 를 설명하고, (iii) 0.4 m/s passive 와 50–100 m/s saltatory 가 *왜 100배 차이* 인지 한 문단으로 답할 수 있어야 한다.
...
피라미드 뉴런의 apical dendrite 는 길이 1 mm, 직경 1 μm. 한쪽 끝 시냅스 입력이 soma 까지 *attenuated + delayed* 로 도착 — 그 정도가 cable theory 의 답이다. 본질: cytoplasm 자체가 저항 ($R_i ≈ 100\,\Omega\cdot$cm) 이라 *옴 강하* 누적 → 먼 점은 덜 충전됨.
...
*직관*: 굵을수록 ($d↑$) axial 저항이 *단면적에 반비례* 로 줄어 멀리 간다. 막 저항 ($R_m↑$) 이 클수록 옆으로 새지 않아 멀리 간다. Cytoplasm 저항 ($R_i↑$) 이 크면 가까이 못 간다.

전형값: $d = 1\,\mu\mathrm{m}$, $R_m = 10^4\,\Omega \cdot \mathrm{cm}^2$, $R_i = 100\,\Omega \cdot \mathrm{cm}$ → $\lambda \approx 500\,\mu\mathrm{m}$.

---

## §4. 정상상태 해 — V(x) = V₀ e^{-x/λ}
...
**실험적 의미.** Dendrite 끝에 EPSP 가 들어와도 soma 에 도달할 때까지 *지수적으로 감쇠*. 1 λ 거리에서 37%, 2λ 에서 14%, 3λ 에서 5%. 따라서 *멀리 있는 시냅스 입력은 단순 합산으로는 spike 를 만들기 어려움* — 이것이 active dendrite (NMDA spike, Ca spike) 의 진화적 동기.
...
*전파 메커니즘* (3 단계): (1) x₀ 에서 AP 발화 → V 가 +30 mV. (2) Cable 전류가 x₀+Δx 의 막을 *임계 위로* depolarize. (3) Voltage-gated Na 채널 열림 → 새 AP. 반복.

**핵심 통찰.** 전파되는 것은 "전압 자체" 가 아니라 *AP 발화 사건*. 매 위치에서 새 AP 가 *재생* 되므로 진폭 감쇠 없이 끝까지 도달. *속도* (Slide L6 p.20): 직경 1 μm, unmyelinated → ~0.4 m/s. 좌골신경 1 m 가 2.5 초 — 너무 느림.
...
**왜 빠른가?** 두 효과: (1) Internode 의 $c_m$ 이 myelin 두께에 반비례 → 충전할 전하 적음 → $\tau_m$ 짧음. (2) Internode 의 $r_m$ 증가 → λ 길어짐 → cable 전류가 멀리 도달. AP 가 internode 를 *전선처럼* 빠르게 통과하고 다음 Node 에서 *재점화*. 평균 속도 50–100 m/s.

*속도 비교*: Squid giant axon (직경 500 μm, unmyelinated) 25 m/s, 포유류 motor Aα (12–20 μm, myelinated) 80–120 m/s. 포유류는 *얇게 + myelin* 조합으로 같은 부피에 더 많은 빠른 axon 을 packed.
...
$g_{ij}$ 는 i-j 간 axial coupling. NEURON/GENESIS 가 자동 빌드. *선택 기준*: 컴파트먼트 길이 ≪ λ (보통 0.1λ). 1 mm cable, λ = 500 μm → 최소 20 컴파트먼트.
...
**한 줄 요약.** Cable PDE 의 $-(V-V_{rest})$ 항이 공간 감쇠의 본질. 만약 $r_m \to \infty$ (leak 0) 이면 정상상태 해는 $V(x) = V_0$ — *감쇠 없음*, 무한 도달. 실제는 항상 leak 이 있어 λ 가 유한.
...
2. **"굵은 axon 이 항상 빠르다"** — 부분적 사실. λ ∝ √d 이므로 100배 굵게 해야 10배 속도. Myelin 은 같은 직경에서 100배 빠름 — *훨씬 효율적*.
3. **"λ 는 막 시상수 τ_m 의 공간판"** — 별개의 물리. τ_m = R_m C_m (시간), λ = √(d R_m / 4 R_i) (공간). 둘 다 R_m 에 의존하지만 *독립 변수*.
4. **"Multi-compartment 가 더 정확하면 항상 multi 써야"** — 아니다. 컴파트먼트 수가 늘면 ODE 차수도 늘어 *시뮬 시간 ∝ N²*. 질문이 "soma spike 발화" 면 단일 컴파트먼트가 충분. 질문이 "dendritic Ca spike" 라야 multi 가 필요.
5. **"Myelin 은 단순한 절연체"** — 너무 단순. Schwann/oligodendrocyte 가 *능동* 으로 axon 직경 조절, 채널 분포 결정 (Na 는 Node 에 100배 농축). Pathology (MS) 시 saltatory 무너짐 — *수동 cable 전파만* 남아 0.4 m/s 로 회귀.
...
- [ ] Unmyelinated 0.4 m/s vs myelinated 50–100 m/s 의 100배 차이를 *두 메커니즘* 으로 설명한다 (capacitance 감소 + leak 감소).
- [ ] 정상상태 V(x) 측정만으로는 $R_m, R_i$ 가 식별 불가능한 이유를 한 문장으로 설명한다.
- [ ] Multi-compartment 모형의 컴파트먼트 길이를 어떻게 정하는지 한 문장으로 답한다 (≪ λ).
- [ ] λ ≈ 500 μm, AP 속도 0.4 m/s (unmyelinated), 50–100 m/s (myelinated) 의 표준 숫자를 즉시 답한다.
""".strip()
...
if __name__ == "__main__":
    ...
    print(f"L6 v2 cached: {len(L6_SUMMARY)} chars; figures: 5")
