#!/usr/bin/env python3
"""
seed_exemplar_L8_v2.py — Opus 4.7 hand-rewritten L8 summary, v2.

Neural codes — rate, temporal, phase, synchrony + multiplexing.
"""
import json, sys
...
F2 = fig("hippocampal_phase_precession.svg",
         ...
         "그림 2. 해마 place cell 의 phase precession. Theta 진동 (8 Hz) 의 위상에 대해 spike 가 *앞당겨* 발화 — 위치가 place field 입구에서 출구로 이동하면 위상이 한 cycle (360°) 만큼 *후행→선행* 으로 이동. *위치 정보가 위상에 인코딩*.")
...
| 정의 | 평균 방향 | 윈도 | 사용처 |
|---|---|---|---|
| **Spike count rate** | 시간 평균 | T = 100–500 ms | 정상 자극 |
| **Spike density (PSTH)** | 시행 평균 | $\Delta t$ = 1–10 ms | time-varying 자극 |
...
**한 줄 요약.** Spike count 평균은 *수십~수백 ms* 의 윈도가 필요 — 그러나 인간 시각 인식은 *400 ms* 안에 완료 [Slide L8 p.26–28].

*Thorpe et al. (1996)*: 사람이 자연 사진에서 동물 유무 판단을 150 ms 안에 함. 시각 경로 (망막→V1→IT→PFC) 가 7–10 단계인데, 단계당 평균 시간 = 15 ms. Rate code 의 평균 윈도 (50+ ms) *보다 짧음* — 각 뉴런이 *1–2 spike* 만으로 정보 전달.
...
**한 줄 요약.** 망 차원의 진동 (e.g., theta 8 Hz) 이 *내부 시계* 역할. Spike 가 진동 *위상* 에 대해 어떤 시점에 발화하는가 = 부호 [Slide L8 p.35–36].

*해마 place cell — phase precession (O'Keefe & Recce 1993)* [Slide L8 p.39–40]:

쥐가 place field 의 입구로 들어올 때 → place cell 이 theta 의 *후기 위상* 에 발화. 쥐가 field 중앙으로 이동 → spike 가 점점 *이른 위상* 으로 이동 (precession). Field 출구에서는 *초기 위상* 에 발화. 한 traversal 동안 위상이 360° 만큼 *앞당겨진다*.
...
*증거.* 청각피질의 두 뉴런이 4 kHz 톤에 응답할 때, 각자의 firing rate 는 서로 다른 정보; 그러나 두 spike 의 *상대적 시간* 에 *추가* 정보가 있음.
...
*결과*:
- 일정 DC: spike timing 이 trial 마다 **±10–30 ms** 변동 — *unreliable*.
- 변동 noise: spike timing 이 trial 마다 **<1 ms** 일치 — *highly reliable*.

**의미**. 뉴런 자체는 *충분히 정확* (sub-ms). Trial-to-trial 변동성은 ion channel noise 가 아니라 *자극이 평탄해서* — 막이 어떤 phase 에서 임계를 넘는지 random 결정. 자극이 변동하면 막이 *예측 가능한* 시점에서만 임계 넘김 → 정밀.

이는 temporal code 의 *물리적 가능성* 을 뒷받침. *뉴런은 1 ms 정밀도가 가능* — 단 입력에 정보가 있어야 한다.

---

## §9. 변동성과 noise — 보편 부호는 없다

**한 줄 요약.** 같은 spike train 이 다른 뉴런에서는 *다른 정보* 를 의미할 수 있어 *보편적 (universal) 신경 부호는 없다* [Slide L8 p.70–73]. Noise 원천 (Faisal et al. 2008): channel noise (stochastic 개폐), synaptic noise (vesicle release probability ~0.3), input noise (presynaptic 변동). Noise 와 부호의 상호작용이 측정 문제의 핵심.
...
- **Synchrony code**: L4 와 L5/6 사이 spike-time coherence 가 *모든 frequency 범위* 에서 일정.
...
3. **"뉴런이 noisy 하니 정밀 부호 불가능"** — 아니다 (Mainen-Sejnowski). 입력에 정보가 있으면 뉴런은 sub-ms 정밀. Noise 는 자극이 *평탄할 때만* 두드러짐.
...
- [ ] Thorpe (1996) 의 150 ms 시각 인식 결과가 *rate code 의 한계* 를 어떻게 시사하는지 한 문단으로 설명한다.
...
if __name__ == "__main__":
    ...
    print(f"L8 v2 cached: {len(L8_SUMMARY)} chars; figures: 3")
