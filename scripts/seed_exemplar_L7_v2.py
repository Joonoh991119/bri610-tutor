#!/usr/bin/env python3
"""
seed_exemplar_L7_v2.py — Opus 4.7 hand-rewritten L7 summary, v2.

Reduced neuron models — IF, LIF, Izhikevich, multi-compartment HH for DA.
"""
import json, sys
...
F4 = fig("hh_gating_variables.svg",
         ...
         "그림 4. 단순화의 동기. HH 4 개 ODE × N 뉴런 = 4N 차수, *비선형* 결합. LIF 는 1 ODE + threshold rule × N = N 차수, *선형* (subthreshold). 100만 뉴런 망에서 HH 는 슈퍼컴, LIF 는 노트북.")
...
*숫자*: HH 단일 뉴런 1초 시뮬 (Δt = 0.025 ms) → 40,000 step × 4 ODE × 비선형 = 큰 비용. *백만* 뉴런으로 늘리면 슈퍼컴 단위. 두뇌에는 10¹¹ 뉴런이 있다.
...
Sub-threshold 가정 ($m \approx 0$, $n \approx n_∞$ 일정, $h \approx h_∞$ 일정) 하에 active 항이 leak 으로 흡수:
...
*해석.* AP 의 1 ms 모양을 1 줄의 reset 으로 *대체*. 잃는 정보 = AP 모양. 가설: 모양은 *stereotyped* 이므로 잃어도 OK.
...
$$V(t) = V_∞ + (V_{reset} - V_∞)\,e^{-t/\tau_m}, \quad V_∞ = E_L + R_m I_e.$$
...
**발화 임계 조건**: $R_m I_e > V_{th} - E_L$. 그렇지 않으면 $V_∞ < V_{th}$ — 영원히 발화 안 함 ($r = 0$).

*피질 뉴런과 비교* (Slide L7 p.17): 큰 입력에서 LIF 는 *너무 빠르게* 적응 없이 발화 — 실제 뉴런은 첫 spike 가 빠르고 점점 느려짐 (spike-rate adaptation). LIF 는 이 적응을 *놓침*.
...
**기능적 의미** [Slide L7 p.19–22]: (1) **Contrast adaptation** (과포화 방지), (2) **Forward masking** (첫 자극 IAHP 가 두 번째 응답 억제), (3) **Selective attention** (약한 입력은 묻히고 강한 입력만 pop-out).
...
*매개변수*: $a$ = recovery 시간 척도, $b$ = recovery sensitivity, $c$ = V reset (낮으면 burst), $d$ = u jump (크면 fatigue). $v$ 식의 2차 항이 spike upstroke 를 *내장* — LIF reset rule 보다 자연스럽고, ODE 차수 2 (HH 절반). *응용* [Slide L7 p.41–44]: Striatal MSN, DA 뉴런의 D1/D2 약물 응답을 (a,b,c,d) 변화로 매핑.
...
| 망 동역학 (oscillation, sync) — 100K+ 뉴런 | LIF or Izhikevich | 비용 < 정확도 |
...
1. **"LIF 가 HH 보다 부정확"** — 부분적. Sub-threshold 영역에선 LIF = HH (정의상). 부정확한 곳은 *spike 자체* (1 ms 윈도). 시간 분해능 > 1 ms 의 망 시뮬에선 무관.
...
- [ ] 발화 임계 조건 $R_m I_e > V_{th} - E_L$ 을 식에서 즉시 도출한다.
...
if __name__ == "__main__":
    ...
    print(f"L7 v2 cached: {len(L7_SUMMARY)} chars; figures: 5")
